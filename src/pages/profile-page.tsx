...
function Timeline({ items }: { items: TimelineItem[] }) {
  const { locale, t } = useLocale();
  return <div className="profile-timeline">{items.map((item, index) => <article key={item.id}><div className="profile-timeline-line" aria-hidden="true"><span /></div><div className="profile-timeline-period"><span>{String(index + 1).padStart(2, "0")}</span><p>{item.period}</p></div><div className="profile-timeline-body"><div className="profile-timeline-title"><h3>{t(item.title)}</h3><span>{item.status === "ongoing" ? (locale === "ko" ? "재학 중" : "In progress") : (locale === "ko" ? "완료" : "Completed")}</span></div><p className="profile-timeline-org">{t(item.organization)}</p><p>{t(item.description)}</p><ul className="tag-list">{item.tags.map((tag) => <li key={tag}>#{tagLabel(tag, locale)}</li>)}</ul></div></article>)}</div>;
}
